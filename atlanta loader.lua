	...
	if not library.font then
		pcall(function()
			if type(writefile) == "function" and type(getcustomasset) == "function" then
				writefile("ArialBold.ttf", game:HttpGet("https://raw.githubusercontent.com/ZombieZach12/atlanta/main/ArialBold.ttf"))
...
section:toggle({name = "Disable Glow", flag = "Disable Glow", callback = library.update_glows})
				...
				section:toggle({name = "Tabs Mode", flag = "window_mode_toggle", callback = function(bool)
					flags["window_mode"] = bool and "Tabs" or "All at once"
					library:update_window_mode()
				end})
